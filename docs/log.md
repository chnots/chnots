...
经过几个月的沉淀，与其中若有若无的考虑，最后应该是这样的。

## Domain

笔记的隔离标准是 domain，就是命名域。domain 中的访问与发布关系为：
...
上面表格中的高等级低等级应该有专门的页面进行维护。比如说 private 的等级要比 public 高，因为一个人既然可以访问私有的，那么就更可以访问公有的。
...
发布与访问正好相反是因为防止手滑将私密的内容发布到公有领域。

## Chnot

组成笔记的单元为 NestedChnot，组成 NestedChnot 的基本单位为 Chnot。其中 Chnot 是数据库中的记录单位，NestedChnot 是 Chnot 的树。

NestedChnot 只是支持有限操作的树，只支持新增与删除，不支持 Chnot 的移动。

Chnot 中的存储内容为 Markdown。虽然我是 Org-Mode 的粉丝，但是 Markdown 的支持更为普遍。如果将 Org-Mode 的部分特性迁移进来，我也是可以接受的。

Chnot 中还需要支持一些其他内容，比如代办提醒、标签、双向链接与特殊内容的渲染，比如表格（复杂，类似 excel）、白板等。

之所以使用 NestedChnot，是因为单个 Chnot 的版本管理。

目前要做的是 NestedChnot 的增删改查。

### 增
增加主要指的是使用 Chnot 的增加，主要涉及
