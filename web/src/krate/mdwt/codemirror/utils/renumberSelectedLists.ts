...
const renumberSelectedLists = (state: EditorState): TransactionSpec => {
  ...
  const handleLines = (linesToHandle: Line[]) => {
    ...
    let firstListNumber = Number(
      listItemRegex.exec(linesToHandle[0].text)?.[2],
    );
    ...
    for (const line of linesToHandle) {
      ...
      let currentGroupIndentLength = tabsToSpaces(
        state,
        currentGroupIndentation,
      ).length;
      ...
      if (indentIncreased) {
        ...
        listNumberStack.push({
          nextListNumber,
          indentationLength: currentGroupIndentLength,
        });
        nextListNumber = 1;
  ...
  for (const selection of state.selection.ranges) {
    ...
    if (listLines.length > 0) {
      ...
      selectedListRanges.push(EditorSelection.range(fromLine.from, toLine.to));
    }
  }
