  ...
  for (const { from, to } of view.visibleRanges) {
    ensureSyntaxTree(view.state, to)?.iterate({
      ...
      enter: (node) => {
        ...
        if (Object.hasOwn(nodeNameToLineDecoration, node.name)) {
          const decoration = nodeNameToLineDecoration[node.name];
          addDecorationToLines(viewFrom, viewTo, decoration);
          blockDecorated = true;
        }

        if (Object.hasOwn(nodeNameToMarkDecoration, node.name)) {
        ...
        if (blockDecorated && Object.hasOwn(multilineNodes, node.name)) {
...
const decoratorExtension = ViewPlugin.fromClass(
  class {
